...
const App = () => {
  //this function will upload the profile picture of student when select it

  const uploadImage = image => {
    const reference = storage().ref(`Users/${userID}/test.png`);
    reference.putFile(image).then(() => {
      console.log('image uploaded successfully...');
    });
    reference.putFile(image).catch(error => {
      console.log('Something happend while image upload process: ' + error);
    });
  };
  ...
  let [school, setSchool] = useState('20 nömrəli məktəb');
  let [home, setHome] = useState('Ev ünvanını seçin');
    ...
    if (
      ...
      reference.once('value').then(snapshot => {
        ...
        database()
          ...
          .set(userData)
          .then(() => {
            console.log(`New user ${userID} is data saved!`);
          });
      });
    } else {
          ...
          <View
            ...
            <View
              ...
              <View
                ...
                <Image
                  source={require('./src/images/address.png')}
            ...
            <View
              ...
              <View
                ...
                <Image
                  source={require('./src/images/school.png')}
                  style={{width: 50, height: 50}}
                />
                <Text style={{fontSize: 14, color: 'white'}}>{school}</Text>
              </View>
            </View>
          </View>
        </LinearGradient>
